...
import { useUser } from '../context/UserContext'; // Імпорт

const ProductCard = ({ product }) => {
  const { addToCart } = useCart();
  const { user, toggleWishlist } = useUser(); // Дістаємо функцію

  // Перевіряємо, чи товар вже лайкнутий
  const isLiked = user?.wishlist?.some(p => p.id === product.id);
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg hover:shadow-xl transition duration-300 overflow-hidden border border-gray-100 flex flex-col h-full relative group">
      ...
      <button 
        onClick={(e) => {
            e.preventDefault(); // Щоб не переходити по посиланню, якщо картка обгорнута
      ...
      <div className="p-5 flex flex-col flex-grow">
        <h3 className="text-xl font-bold text-gray-800 mb-2">
            <Link to={`/product/${product.id}`} className="hover:text-green-600 transition">
                {product.name}
            </Link>
        </h3>
