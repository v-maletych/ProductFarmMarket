...
const ProductCard = ({product}) => {
    ...
    return (
        <div
            className="bg-white rounded-xl shadow-lg hover:shadow-xl transition duration-300 overflow-hidden border border-gray-100 flex flex-col h-full relative group">
            <button
                onClick={(e) => {
                    e.preventDefault();
            ...
            >
                <svg xmlns="http://www.w3.org/2000/svg"
                     className={`h-6 w-6 transition ${isLiked ? 'text-red-500 fill-current' : 'text-gray-400'}`}
                     viewBox="0 0 24 24" stroke="currentColor" strokeWidth={isLiked ? 0 : 2}>
            ...
            <div className="h-48 overflow-hidden relative">
                <Link to={`/product/${product.productId}`}>
                    ...
                    />
                    <div
                        className="absolute inset-0 bg-black opacity-0 group-hover:opacity-5 transition duration-300"></div>
                </Link>
            </div>
            ...
            <div className="p-5 flex flex-col flex-grow">
                ...
                <div className="flex items-center justify-between mt-auto">
                    <div>
                        <span
                            className="text-2xl font-bold text-green-600">{product.price?.toFixed(2) || '0.00'} ₴</span>
