    ...
    return (
        <>
            <header className="bg-white shadow-md sticky top-0 z-40 w-full">
                <div
                    className="relative w-full px-6 md:px-10 py-4 flex flex-wrap md:flex-nowrap justify-between items-center gap-4">
                    <div className="flex items-center gap-4 z-10 shrink-0">
                        <button onClick={() => setIsMenuOpen(true)}
                                className="p-1 text-gray-600 hover:text-green-600 transition focus:outline-none">
                        ...
                        <Link to="/"
                              className="text-2xl md:text-3xl font-black text-green-600 tracking-tight whitespace-nowrap">FarmMarket</Link>
                    </div>
                    <div className="relative w-full md:max-w-md order-last md:order-none" ref={searchRef}>
                        <div className="relative">
                            ...
                            />
                            <svg className="w-5 h-5 text-gray-400 absolute left-3 top-2.5" fill="none"
                        ...
                        {showResults && (
                            <div
                                className="absolute top-full left-0 w-full bg-white shadow-xl rounded-xl mt-2 border border-gray-100 max-h-80 overflow-y-auto z-50">
                                    ...
                                    <ul>
                                        {searchResults.map(product => (
                                            <li key={product.productId}
                                                ...
                                                <img src={product.image || 'https://placehold.co/40x40'}
                                                     alt={product.name} className="w-10 h-10 rounded object-cover"/>
                                                <div>
                    ...
                    <div className="flex items-center gap-4 md:gap-6 shrink-0 ml-auto md:ml-0">
                        <nav className="hidden lg:flex items-center gap-6 font-medium text-gray-600">
                            ...
                            {isFarmerOrAdmin && (
                                <Link to="/farmer-dashboard"
                                      className="text-green-700 font-bold hover:text-green-900 transition flex items-center gap-1 bg-green-50 px-3 py-1 rounded-full">
                                    👨‍🌾 Панель Фермера
                                </Link>
                            )}
                        </nav>
                        ...
                        {authData.isAuthenticated ? (
                            <Link to="/profile"
                                ...
                                <div
                                    className="w-9 h-9 bg-green-100 rounded-full flex items-center justify-center text-green-700 font-bold shadow-sm group-hover:bg-green-200 transition">
                                    {userNameForAvatar}
                                </div>
                            </Link>
                        ) : (
                        ...
                        <Link to="/wishlist" className="relative text-gray-600 hover:text-red-500 transition p-1">
                            ...
                            {wishlistCount > 0 && (
                                <span
                                    className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold rounded-full h-4 w-4 flex items-center justify-center border border-white">
                                    {wishlistCount}
                                </span>
                            )}
                        </Link>
                        <Link to="/cart" className="relative text-gray-700 hover:text-green-500 transition p-1">
                            ...
                            {getCartCount() > 0 && (
                                <span
                                    className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold rounded-full h-5 w-5 flex items-center justify-center border-2 border-white">
                                    {getCartCount()}
                                </span>
                            )}
                        </Link>
            ...
            <div
                className={`fixed top-0 left-0 h-full w-80 bg-white z-50 shadow-2xl transform transition-transform duration-300 ease-in-out overflow-y-auto ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="p-5 border-b flex justify-between items-center bg-green-50">
                    ...
                    <button onClick={closeMenu} className="text-gray-500 hover:text-red-500">
                    ...
                    <div className="mb-6 pb-6 border-b md:hidden space-y-2">
                        {authData.isAuthenticated ? (
                            <Link to="/profile" onClick={closeMenu}
                                  className="flex items-center gap-3 p-3 bg-green-50 rounded-lg text-green-800 font-bold mb-4">👤
                                Особистий кабінет</Link>
                        ) : (
                            <Link to="/login" onClick={closeMenu}
                                  className="flex items-center gap-3 p-3 bg-gray-100 rounded-lg text-gray-700 font-bold mb-4">🔑
                                Увійти</Link>
                        )}

                        <Link to="/" onClick={closeMenu}
                              className="block py-2 text-lg font-medium text-gray-700 hover:text-green-600">🏠
                            Головна</Link>
                        <Link to="/products" onClick={closeMenu}
                              className="block py-2 text-lg font-medium text-gray-700 hover:text-green-600">📦 Весь
                            Каталог</Link>
                        {isFarmerOrAdmin && (
                            <Link to="/farmer-dashboard" onClick={closeMenu}
                                  className="block py-2 text-lg font-bold text-green-700 hover:text-green-900 bg-green-50 px-2 rounded">
                                👨‍🌾 Панель Фермера
                            </Link>
                        )}

                        <Link to="/wishlist" onClick={closeMenu}
                              className="block py-2 text-lg font-medium text-gray-700 hover:text-red-500">❤️
                            Обране</Link>
                    </div>
                        ...
                        <ul className="space-y-1">
                            {categories.map((cat) => (
                                <li key={cat.categoryId}>
                                    <Link to={`/products/${cat.name.toLowerCase()}`} onClick={closeMenu}
                                          className="flex items-center gap-3 p-3 rounded-lg hover:bg-green-50 text-gray-700 hover:text-green-700 transition">
                                        <div
                                            className="w-8 h-8 rounded-full bg-gray-200 object-cover flex items-center justify-center text-sm">{cat.name.charAt(0)}</div>
