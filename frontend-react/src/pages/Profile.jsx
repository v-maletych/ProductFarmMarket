...
const OrderItem = ({ order }) => {
    ...
    return (
        <li className="border border-gray-100 rounded-2xl overflow-hidden bg-white transition hover:shadow-lg group mb-4">
            ...
            <div onClick={() => setIsOpen(!isOpen)} className="p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 cursor-pointer bg-white hover:bg-gray-50 transition duration-300">
                <div>
                    <div className="flex items-center gap-3">
                        <div className="bg-green-100 text-green-600 w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg">
                            📦
                        </div>
                        <div>
                            <p className="font-bold text-gray-800 text-lg">Замовлення #{order.orderId}</p>
                            <span className={`inline-block mt-1 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${order.deliveryStatus === 'COMPLETED' ? 'bg-green-100 text-green-600' : 'bg-yellow-100 text-yellow-600'}`}>
                ...
                <div className="flex items-center gap-6">
                    <div className="text-right">
                        ...
                        <span className="block text-xl font-bold text-gray-900">
                            {order.totalAmount ? order.totalAmount.toFixed(2) : '0.00'} <span className="text-sm font-normal text-gray-500">грн</span>
                        </span>
                    </div>
                    <div className={`w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 text-gray-400 transition-transform duration-300 ${isOpen ? 'rotate-180 bg-green-50 text-green-600' : ''}`}>
            ...
            {isOpen && (
                <div className="p-6 border-t border-gray-100 bg-gray-50/50 animate-fadeIn">
                    <div className="mb-6 p-4 bg-white rounded-xl border border-gray-100 shadow-sm flex items-start gap-3">
                        <div className="mt-1 text-blue-500">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                        </div>
                        <div>
                            <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-1">Адреса доставки</h4>
                            <p className="text-sm font-medium text-gray-700">{order.deliveryAddress || 'Самовивіз / Не вказана'}</p>
                        </div>
                    </div>

                    <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-3 pl-1">🛒 Товари в чеку</h4>

                    {order.orderItems && order.orderItems.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {order.orderItems.map((item, index) => (
                                <div key={index} className="flex items-center gap-3 p-3 bg-white border border-gray-100 rounded-xl shadow-sm">
...
const getRoleDisplay = (role) => {
    switch (role) {
        case 'FARMER': return { text: 'Продавець (Фермер)', color: 'text-lime-600', bg: 'bg-lime-100' };
        case 'CUSTOMER': return { text: 'Покупець (Клієнт)', color: 'text-green-600', bg: 'bg-green-100' };
        case 'ADMIN': return { text: 'Адміністратор', color: 'text-red-600', bg: 'bg-red-100' };
        default: return { text: 'Користувач', color: 'text-gray-500', bg: 'bg-gray-100' };
    }
};
            ...
            <div className="container mx-auto px-4 max-w-5xl">
                ...
                <div className="bg-white rounded-[2rem] shadow-xl overflow-hidden border border-gray-100">
                    ...
                    <div className="px-8 pb-10">
                        {/* Верхня частина: Аватар + Ім'я + Кнопка */}
                        <div className="flex flex-col md:flex-row items-center md:items-end -mt-20 mb-10 gap-6 relative z-10">

                            {/* Аватар */}
                            <div className="w-40 h-40 rounded-full border-[6px] border-white shadow-2xl bg-white text-green-600 flex items-center justify-center text-6xl font-black relative overflow-hidden group">
                                <div className="absolute inset-0 bg-green-50 group-hover:bg-green-100 transition duration-500"></div>
                                <span className="relative z-10">{user.firstName?.charAt(0).toUpperCase()}</span>
                            </div>

                            {/* Текст (якщо не редагуємо) */}
                            {!isEditing && (
                                <div className="text-center md:text-left mb-2 flex-grow">
                                    ...
                                    <div className="flex items-center justify-center md:justify-start gap-3 mt-2">
                                        ...
                                        <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide ${roleInfo.bg} ${roleInfo.color}`}>
                                            {roleInfo.text}
                                        </span>
                                    </div>
                                </div>
                            ...
                            <div className="animate-fadeIn max-w-4xl mx-auto">
                                <div className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
                                    <div className="bg-gray-50 px-8 py-4 border-b border-gray-100 flex justify-between items-center">
                                        ...
                                        <button onClick={() => setIsEditing(false)} className="text-gray-400 hover:text-red-500 transition">
                                    ...
                                    <div className="p-8">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                            ...
                                            <div className="relative group">
                                                <label className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 block group-focus-within:text-green-600 transition">Ім'я</label>
                                                <div className="relative">
                                                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-400 group-focus-within:text-green-500 transition">
                                            ...
                                            {/* Прізвище */}
                                            <div className="relative group">
                                                <label className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 block group-focus-within:text-green-600 transition">Прізвище</label>
                                                <div className="relative">
                                                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-400 group-focus-within:text-green-500 transition">
                                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clipRule="evenodd" /></svg>
                                                    </div>
                                            ...
                                            {/* Телефон */}
                                            <div className="relative group">
                                                <label className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 block group-focus-within:text-green-600 transition">Телефон</label>
                                                <div className="relative">
                                                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-400 group-focus-within:text-green-500 transition">
                                            ...
                                            <div className="relative opacity-60">
                                                <label className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 block">Email (не змінюється)</label>
                                                <div className="relative">
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 mt-8 w-full animate-fadeIn">
                                ...
                                <div className="lg:col-span-2">
                                    <div className="flex items-center justify-between mb-6">
                                        ...
                                        <span className="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-xs font-bold">{orders.length}</span>
                                    </div>
                                    ...
                                    {orders.length > 0 ? (
                                        ...
                                        <div className="flex flex-col items-center justify-center py-16 bg-white border-2 border-dashed border-gray-200 rounded-3xl text-gray-400">
