    ...
    return (
        <div className="container mx-auto px-4 py-10">
            <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                ...
                <div
                    className="bg-yellow-50 border border-yellow-200 px-6 py-2 rounded-xl flex items-center gap-3 shadow-sm">
                    <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
                <div>
                    <h2 className="text-xl font-bold text-gray-700 mb-4 flex items-center gap-2">
                        📦 Вхідні замовлення <span
                        className="bg-red-100 text-red-600 text-xs px-2 py-1 rounded-full">{incomingOrders.length}</span>
                    </h2>

                    <div
                        className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden max-h-[600px] overflow-y-auto">
                            ...
                            <div className="divide-y divide-gray-100">
                                {incomingOrders.map(order => (
                                    <div key={order.orderId} className="group">
                                        ...
                                        >
                                            <div>
                                                <div className="font-bold text-gray-800">#{order.orderId} <span
                                                    className="font-normal text-gray-500 text-sm">від {new Date(order.orderDate).toLocaleDateString()}</span>
                                                </div>
                                                <div
                                                    className="text-sm text-gray-600">{order.user?.firstName} {order.user?.lastName}</div>
                                            </div>
                                        ...
                                        {expandedOrderId === order.orderId && (
                                            <div
                                                className="bg-blue-50 p-4 text-sm animate-fadeIn border-t border-blue-100">
                                                <div className="mb-3">
                                                        ...
                                                        <li key={idx} className="flex justify-between items-center">
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                <span>{item.product?.name} <span
                                                                    className="text-gray-400">x{item.quantity}</span></span>
                                                            </div>
                                                            <span
                                                                className="font-bold">{(item.price * item.quantity).toFixed(2)} грн</span>
                                                        </li>
                                                    ))}
                                                </ul>
                                            </div>
                ...
                <div>
                    ...
                    <div
                        className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 max-h-[600px] overflow-y-auto">
                        {myReviews.length === 0 ? (
                                    ...
                                    <li key={review.reviewId} className="border-b border-gray-100 pb-4 last:border-0">
                                        <div className="flex justify-between items-start mb-1">
                                            ...
                                            <div className="flex text-yellow-400 text-xs">
                                                {[...Array(5)].map((_, i) => (
                                                    <FaStar key={i}
                                                            className={i < (review.raiting === 'FIVE' ? 5 : review.raiting === 'FOUR' ? 4 : review.raiting === 'THREE' ? 3 : review.raiting === 'TWO' ? 2 : 1) ? 'text-yellow-400' : 'text-gray-200'}/>
                                                ))}
                                            </div>
                                        </div>
            ...
            <div className="border-t pt-8">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <Link to="/farmer/add-product"
                          className="bg-green-600 text-white px-4 py-2 rounded-lg font-bold shadow hover:bg-green-700 transition flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        ...
                        <div key={product.productId}
                            ...
                            <div className="h-48 w-full relative overflow-hidden bg-gray-100">
                                {product.image ? (
                                    ...
                                    <div className="flex items-center justify-center h-full text-gray-300">Немає
                                        фото</div>
                                )}

                                <div className="absolute top-2 right-2">
                                     <span
                                         className={`text-[10px] px-2 py-1 rounded-full font-bold uppercase shadow-sm ${product.inStock ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                            ...
                            <div className="p-4 flex-grow flex flex-col">
                                ...
                                <div className="mt-auto flex border rounded-lg overflow-hidden">
                                    <Link to={`/farmer/edit-product/${product.productId}`}
                                          className="flex-1 py-2 text-center bg-gray-50 text-blue-600 text-sm font-bold hover:bg-blue-50 transition border-r border-gray-200">
                                        Змінити
                                    </Link>
                                    <button onClick={() => handleDeleteProduct(product.productId)}
                                            className="flex-1 py-2 text-center bg-gray-50 text-red-600 text-sm font-bold hover:bg-red-50 transition">
                                        Видалити
                                    </button>
                                </div>
                            </div>
                    ...
                    <Link to="/farmer/add-product"
                          className="border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center p-6 text-gray-400 hover:border-green-500 hover:text-green-500 transition cursor-pointer min-h-[300px]">
