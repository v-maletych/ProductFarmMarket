    ...
    if (error || !product) {
        return (
            <div className="text-center py-20">
                ...
                <button onClick={() => navigate('/products')} className="mt-4 text-green-600 underline">Повернутися в
                    каталог
                </button>
            </div>
        );
    ...
    return (
        <div className="container mx-auto px-4 py-10">
            <button onClick={() => navigate(-1)}
                    className="mb-6 text-gray-500 hover:text-green-600 flex items-center gap-2 transition">← Назад
            </button>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-16">
                <div className="bg-white rounded-2xl shadow-lg overflow-hidden p-4 border border-gray-100">
                    <img src={product.image || 'https://placehold.co/800x600'} alt={product.name}
                ...
                <div className="flex flex-col justify-center">
                    ...
                    {product.ownerId && (
                        <Link to={`/seller/${product.ownerId}`}
                              className="flex items-center gap-2 mb-3 text-sm text-gray-500 hover:text-green-600 transition group w-fit">
                            <span className="bg-gray-100 group-hover:bg-green-100 p-1 rounded-full text-xs">👤</span>
                            <span className="font-semibold">Продавець: <span
                                className="underline group-hover:no-underline">{product.ownerName}</span></span>
                        </Link>
                    )}
                    ...
                    <div className="flex items-center gap-6 mb-8">
                        <span
                            className="text-4xl font-bold text-gray-900">{product.price?.toFixed(2) || '0.00'} грн</span>
                        <span className="text-gray-500 text-xl">/ {product.unit || 'кг'}</span>
                    </div>

                    <div className="flex gap-4">
                        <button onClick={() => addToCart(product)}
                                className="flex-1 bg-green-600 hover:bg-green-700 text-white font-bold py-4 px-8 rounded-xl shadow-lg transition transform hover:-translate-y-1 flex justify-center items-center gap-2">
            ...
            <div className="bg-white rounded-3xl shadow-lg p-6 md:p-10 border border-gray-100">
                ...
                {authData.isAuthenticated ? (
                            ...
                            <div className="mb-4 flex items-center">
                                ...
                                {[5, 4, 3, 2, 1].map(r => (
                                    <label key={r} className="inline-flex items-center mr-3 cursor-pointer">
                                        ...
                                        />
                                        <div
                                            className={`flex items-center p-1 rounded ${newReview.raitingValue === r ? 'bg-yellow-100 ring-1 ring-yellow-400' : ''}`}>
                    ...
                    <div className="mb-10 p-4 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 rounded-lg">
                        Будь ласка, <button onClick={() => navigate('/login', {state: {from: `/product/${productId}`}})}
                                            className="font-semibold underline hover:text-yellow-900">увійдіть</button>,
                        щоб залишити відгук.
                    </div>
                )}
                ...
                <div className="space-y-6">
                    {reviews.length > 0 ? (
                        reviews.map(review => (
                            <div key={review.reviewId || Math.random()}
                                 className="border-b border-gray-100 pb-6 last:border-0">
                                <div className="flex items-start justify-between mb-2">
                                    <div className="flex items-center gap-3">
                                        <div
                                            className="w-10 h-10 bg-green-100 text-green-700 rounded-full flex items-center justify-center font-bold text-lg uppercase">
